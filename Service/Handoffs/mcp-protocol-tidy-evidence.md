...
## Ответ ревьюера (приёмка в `main`, 2026-08-18)

Проверял хост-оркестратор перед merge. Полный набор на объединённом дереве:
`py -3 -m pytest tests -q` → **755 passed, 1 skipped, 79 subtests**, exit 0.
Негоциация перепроверена вживую in-process на всех шести входах
(`2024-11-05`/`2025-03-26`/`2025-06-18` — эхо; `2025-11-25`/`2026-07-28`/нет
поля — `2025-06-18`). Совпало с заявленным.

### 1. `ONE_CLIENT_PER_PROCESS` на reconnect — **не приемлемо, исправлено**

Воспроизвёл на живом loopback-сервере: тот же клиент, тот же `clientInfo`,
второй `initialize` → `ONE_CLIENT_PER_PROCESS` навсегда, до рестарта процесса.
В том же прогоне `tools/list` без `initialize` вообще отработал. То есть гейт
не закрывал ничего — второй клиент мог просто не звать `initialize`, — но
ломал первого при обрыве связи.

Претензия предъявляется теперь имени клиента: `initialize` от другого
`clientInfo.name` по-прежнему `ONE_CLIENT_PER_PROCESS`, тот же клиент
переподключается свободно. Версия в тождество не входит: два экземпляра
одного хоста вполне могут быть разных версий, так что по версии этот случай
всё равно не ловится, а обычный апгрейд хоста начал бы читаться как второй
клиент. Два хоста, одинаково себя называющие, не различимы — так и написано
в коде и в `docs/install/vps.md`; это предупреждение о том, что реестр job'ов
делят двое, а не аутентификация. Аутентификация — bearer.

Тесты: `test_a_second_different_client_is_refused_as_one_process_per_client`,
`test_the_same_client_reconnecting_is_not_a_second_client`,
`test_an_upgraded_host_is_still_the_same_client`.

### 2. latest = `2025-06-18`, а не `2025-11-25` — **оставлено, но причина другая**

Обоснование в R1 («не умеем `2025-11-25` из-за HTTP MUST Origin/405/version
header») к продукту не относится: MUST'ы транспортные, а продуктовый транспорт
здесь stdio, где их нет. Настоящая причина остаться на ревизию назад проще и
честнее: объявить ревизию — значит заявить её набор возможностей целиком, а
что `2025-11-25` добавляет сверх транспорта, никто не проверял. Заявляем то,
что можем защитить. Открытый пункт: сверить `2025-11-25` по пунктам и решить
предметно, а не по транспортному аргументу.

### 3. Гасить `--transport http` полностью — **нет**

Опция остаётся: оператор, который сознательно поднял её за TLS, — это
описанный сценарий VPS, а не случайность. Токен обязателен везде, bind по
умолчанию loopback, не-loopback требует отдельного флага. Остаточный риск R2
(держатель bearer получает тот же `tools/list`, что и stdio) записан в
`docs/SECURITY.md` как есть, без вида, что это ACL.

### Что осталось незакрытым

- Linux systemd / Caddy / публичный DNS не гонялись ни в одном проходе. В
  `docs/install/vps.md` это помечено разделом «Not verified and why» — при
  первом реальном VPS проверить оттуда каждую команду.
- `tools/call` не требует `initialize`. Это расходится со спецификацией
  lifecycle и делает гейт одного клиента предупреждающим. Чинить — отдельная
  задача: HTTP станет строже stdio, и это надо решать осознанно.
